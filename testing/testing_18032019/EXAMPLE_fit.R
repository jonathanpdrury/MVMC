#This script shows how to fit the DDexp model
...
counter=1
for(i in 1:20){ #for 20 trees

	for(j in 2:2){ #for 1 sig2 matrices
	
		for(k in 1:1){ #for 1 slope matrices
		...
		ddm.sig2_1<-exp(ddm.fit$inferredParams[3])^2 + (ddm.fit$inferredParams[7]^2)
		ddm.sig2_2<-exp(ddm.fit$inferredParams[4])^2 + (ddm.fit$inferredParams[7]^2)
		ddm.sig2_cov<-(ddm.fit$inferredParams[7]) * (exp(ddm.fit$inferredParams[3])+exp(ddm.fit$inferredParams[4]))
		...
		res.mat[counter,]<-int
		print(int)
		write.csv(res.mat,file="~/Dropbox/*.csv")
		counter=counter+1
		}
