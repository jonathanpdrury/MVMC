...
createModel_DDexp_MV <- function(tree){
        ...
        #&& ((abs(params[7]) <= exp(params[3])) || (abs(params[7]) <= exp(params[4]))))
        #&& (((abs(params[7])*exp(params[8]*length(tree$tip.label))) <= (exp(params[4])*exp(params[6]*length(tree$tip.label)))) || ((abs(params[7])*exp(params[8]*length(tree$tip.label))) <= (exp(params[3])*exp(params[5]*length(tree$tip.label))))) &&
        #constraints <- function(params) return(params[3]<Inf && params[3] > -Inf && params[4]<Inf && params[4] > -Inf && params[5]<Inf && params[5] > -Inf && all(sign(eigen(matrix(c(exp(params[3]),params[7],params[7],exp(params[4])),nrow=2))$values)!=-1)) 
        #&& all(sign(eigen(matrix(c(exp(params[3])*exp(params[5]*length(tree$tip.label)),params[7]*exp(params[8]*length(tree$tip.label)),params[7]*exp(params[8]*length(tree$tip.label)),exp(params[4])*exp(params[6]*length(tree$tip.label))),nrow=2))$values)!=-1))
