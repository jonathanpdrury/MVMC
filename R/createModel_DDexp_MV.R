...
createModel_DDexp_MV <- function(tree){
        ...
        aAGamma <- function(i, params){
            ...
            matrixGamma <- function(t) return(rbind(cbind((exp(params[3])*exp((params[5]/2)*length(vectorU)))*diag(vectorU), (exp(params[7])*exp((params[8]/2)*length(vectorU)))*diag(vectorU)),cbind( (exp(params[7])*exp((params[8]/2)*length(vectorU)))*diag(vectorU),(exp(params[4])*exp((params[6]/2)*length(vectorU)))*diag(vectorU))))
