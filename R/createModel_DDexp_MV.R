...
createModel_DDexp_MV <- function(tree){
        ...
        #constraints <- function(params) return(params[3]<Inf && params[3] > -Inf && params[4]<Inf && params[4] > -Inf && params[5]<Inf && params[5] > -Inf  && ((abs(params[7]) <= exp(params[3])) || (abs(params[7]) <= exp(params[4]))) && (((abs(params[7])*exp(params[8]*length(tree$tip.label))) <= (exp(params[4])*exp(params[6]*length(tree$tip.label)))) || ((abs(params[7])*exp(params[8]*length(tree$tip.label))) <= (exp(params[3])*exp(params[5]*length(tree$tip.label))))))
