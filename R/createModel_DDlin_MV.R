...
createModel_DDlin_MV <- function(tree){
    

        comment <- "Multivariate_DDlin Model"
        paramsNames <- c("m0_1", "m0_2", "logsigma0_1","logsima0_2","b_1", "b_2","sigma0_cov","b_cov")
        ...
        aAGamma <- function(i, params){
            ...
            matrixGamma <- function(t) return(rbind(cbind(sqrt((exp(params[3])^2)+(params[5]*length(vectorU)))*diag(vectorU),sqrt(((params[7])^2)+(params[8]*length(vectorU)))*diag(vectorU)),cbind(sqrt(((params[7])^2)+(params[8]*length(vectorU)))*diag(vectorU),sqrt((exp(params[4])^2)+(params[6]*length(vectorU)))*diag(vectorU))))
