...
createModel_DDexp_MV <- function(tree){
        ...
        paramsNames <- c("m0_1", "m0_2", "sigma0_1","sima0_2","r_1", "r_2","sigma0_cov","r_cov")
        params0 <- c(0, 0, 1,1,-0.1,-0.1,1,0)
        ...
        aAGamma <- function(i, params){
        	...
        	sig2.matrix <- sym.par(c(params[3],params[4],params[7]))
        	slope.matrix <- sym.par(c(params[5],params[6],params[8]))
            vectorU <- getLivingLineages(i, eventEndOfPeriods)
            vectorA <- function(t) return(rep(0, length(vectorU)*2))
            #matrixGamma <- function(t) return(rbind(cbind((exp(params[3])*exp((params[5]/2)*length(vectorU)))*diag(vectorU), ((params[7])*exp((params[8]/2)*length(vectorU)))*diag(vectorU)),cbind( ((params[7])*exp((params[8]/2)*length(vectorU)))*diag(vectorU),(exp(params[4])*exp((params[6]/2)*length(vectorU)))*diag(vectorU))))
            matrixGamma <- function(t) return(rbind(cbind((sig2.matrix[1,1]*exp((slope.matrix[1,1]/2)*length(vectorU)))*diag(vectorU), (sig2.matrix[1,2]*exp((slope.matrix[1,2]/2)*length(vectorU)))*diag(vectorU)),cbind( (sig2.matrix[2,1]*exp((slope.matrix[2,1]/2)*length(vectorU)))*diag(vectorU),(sig2.matrix[2,2]*exp((slope.matrix[2,2]/2)*length(vectorU)))*diag(vectorU))))
		...
		constraints <- function(params) return(params[3]<Inf && params[3] > -Inf && params[4]<Inf && params[4] > -Inf && params[5]<Inf && params[5] > -Inf &&all(sign(eigen(sym.par(c(params[3],params[4],params[7]))*exp(sym.par(c(params[5],params[6],params[8]))*length(tree$tip.label)))$values)!=-1))
