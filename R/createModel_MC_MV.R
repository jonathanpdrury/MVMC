...
createModel_MC_MV <- function(tree){
    

        comment <- "Multivariate_MC Model"
        paramsNames <- c("m0_1", "m0_2", "logsigma0_1","logsima0_2","sigma0_cov12","S_1","S_2","S_cov12","S_cov21")
        params0 <- c(0, 0, log(1),log(1),0,0,0,0,0)
		ntraits<-2
        ...
        aAGamma <- function(i, params){
            ...
            matrixA <- rbind(cbind(params[6]*diag(vectorU) - (params[6]/sum(vectorU)) * outer(vectorU,vectorU),params[8]*diag(vectorU) - (params[8]/sum(vectorU)) * outer(vectorU,vectorU)),cbind(params[9]*diag(vectorU) - (params[9]/sum(vectorU)) * outer(vectorU,vectorU),params[7]*diag(vectorU) - (params[7]/sum(vectorU)) * outer(vectorU,vectorU)))
             
            return(list(a=vectorA, A=matrixA, Gamma=matrixGamma))
        }

        constraints <- function(params) return(params[6]<=0 && params[7]<=0 && all(sign(eigen(matrix(c(exp(params[3]),params[5],params[5],exp(params[4])),nrow=2))$values)!=-1))
